...
import { Sparkles, Gift, Flame, Calendar, ShoppingBag, Clock, CheckCircle2, ArrowRight } from 'lucide-react';
...
export default function OffersPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fc] relative overflow-hidden font-sans pb-32">
      ...
      <div className="absolute top-48 left-10 w-80 h-80 bg-amber-100/30 rounded-full blur-3xl pointer-events-none -z-0" />
      ...
      <div className={`relative z-10 max-w-7xl mx-auto px-6 md:px-12 pt-16 transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-3">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-800 font-extrabold text-xs tracking-wider uppercase shadow-xs">
            <Sparkles className="w-3.5 h-3.5 text-amber-500" /> Exclusive Savings & Bundles
          </span>
        ...
        <div className="flex items-center justify-center gap-2.5 overflow-x-auto pb-4 mb-12 scrollbar-none">
          <button
            ...
            className={`px-5 py-2.5 rounded-full text-xs font-bold transition-all cursor-pointer flex items-center gap-2 ${
              activeFilter === 'ALL'
                ...
                : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200/80 shadow-xs'
            }`}
          >
          ...
          <button
            type="button"
            onClick={() => setActiveFilter('SUPER_SAVERS')}
            className={`px-5 py-2.5 rounded-full text-xs font-bold transition-all cursor-pointer flex items-center gap-2 ${
              activeFilter === 'SUPER_SAVERS'
                ? 'bg-[#103e2c] text-white shadow-md'
                : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200/80 shadow-xs'
            }`}
          >
            <Gift className="w-3.5 h-3.5 text-emerald-500" />
            Super Saver Bundles ({superSaverCombos.length})
          </button>
          <button
            type="button"
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-3xl p-16 text-center border border-gray-100 shadow-sm max-w-xl mx-auto space-y-3">
            ...
            <p className="text-xs text-gray-400">Check back shortly as new AI smart bundles and seasonal offers are updated daily!</p>
          </div>
        ) : (
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-2 border-b border-gray-200/60 pb-4">
                  <div>
                    <span className="text-[10px] font-black uppercase tracking-wider text-amber-700 bg-amber-50 px-2 py-0.5 rounded border border-amber-200">
                      ⚡ AI Recommended • Admin Approved
                    </span>
                    <h2 className="text-2xl md:text-3xl font-black text-gray-900 mt-1">Smart Clearance Combos</h2>
                    <p className="text-gray-500 text-xs">High-demand pairing combos specially designed for maximum customer savings.</p>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {aiApprovedCombos.map(combo => (
                    <div key={combo.id} className="bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 flex flex-col justify-between group">
                      ...
                      <div className="relative bg-gradient-to-br from-gray-50 to-gray-100/60 p-6 border-b border-gray-100 flex items-center justify-center">
              ...
              <section className="space-y-6">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-2 border-b border-gray-200/60 pb-4">
                  <div>
                    ...
                    <h2 className="text-2xl md:text-3xl font-black text-gray-900 mt-1">Super Saver Combos</h2>
                    <p className="text-gray-500 text-xs">Standard store curated package deals with guaranteed discounts.</p>
                  </div>
                  <span className="text-xs font-bold text-gray-400">
                    {superSaverCombos.length} Combos Available
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {superSaverCombos.map(combo => (
                    <div key={combo.id} className="bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 flex flex-col justify-between group">
                      <div className="relative bg-gradient-to-br from-gray-50 to-gray-100/60 p-6 border-b border-gray-100 flex items-center justify-center">
                        <div className="absolute top-4 left-4 bg-emerald-800 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-wider shadow-sm">
                          Save {combo.discountPercentage.toFixed(0)}%
                        </div>
                        <div className="flex items-center justify-center gap-4 py-4">
                          {combo.items.map((item: any, idx: number) => (
                            <div key={idx} className="flex flex-col items-center">
                              <div className="w-20 h-20 bg-white rounded-2xl p-1.5 shadow-sm border border-gray-200/80 flex items-center justify-center relative group-hover:scale-105 transition-transform">
                                <img
                                  ...
                                  className="w-full h-full object-cover rounded-xl"
                                />
                                <span className="bg-emerald-800 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full absolute -top-2 -right-2">
                                  {item.quantity}x
                                </span>
                              </div>
                              <span className="text-[10px] font-bold text-gray-700 text-center mt-2 line-clamp-1 w-20">
                      ...
                      <div className="p-6 md:p-8 flex flex-col justify-between flex-1 space-y-6">
                        <div>
                          <h3 className="text-xl font-black text-gray-900 mb-2 leading-tight">{combo.name}</h3>
                          <p className="text-gray-500 text-xs line-clamp-2 leading-relaxed">
                            {combo.description || `Buy together and enjoy discount pricing on all bundle items.`}
                          </p>
                        </div>
                        ...
                        <div className="pt-4 border-t border-gray-100 flex items-end justify-between">
                          <div>
                            <div className="flex items-baseline gap-2">
                              <span className="text-xs text-gray-400 line-through font-semibold">Rs. {combo.normalTotalPrice.toFixed(2)}</span>
                              <span className="text-2xl font-black text-emerald-800">Rs. {combo.comboPrice.toFixed(2)}</span>
                            </div>
                            <p className="text-xs font-black text-emerald-700 mt-0.5">
                              Instant Saving: Rs. {(combo.normalTotalPrice - combo.comboPrice).toFixed(2)}
                            </p>
              ...
              <section className="space-y-6">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-2 border-b border-gray-200/60 pb-4">
                  <div>
                    ...
                    <h2 className="text-2xl md:text-3xl font-black text-gray-900 mt-1">Today's Daily Specials</h2>
                    <p className="text-gray-500 text-xs">Limited-time daily promotions available while stocks last.</p>
                  </div>
                  <span className="text-xs font-bold text-gray-400">
                    ...
                    return (
                      <div key={`${discount.id}-${prod.sku}-${idx}`} className="relative h-72 rounded-3xl overflow-hidden group shadow-[0_4px_20px_rgba(0,0,0,0.06)] hover:shadow-2xl transition-all duration-300 border border-gray-100">
                        ...
                        <div className="absolute top-3 right-3">
                           <span className="bg-amber-400 text-gray-900 text-[10px] font-black px-2.5 py-1 rounded-full shadow-sm tracking-wide">
                             {discount.discountValue}% OFF
                           </span>
                        </div>
                        
                        <div className="absolute bottom-0 left-0 w-full p-5 flex flex-col justify-end">
                          <span className="text-amber-300 font-bold text-[10px] mb-1 uppercase tracking-wider line-clamp-1">{discount.label || 'Daily Special'}</span>
